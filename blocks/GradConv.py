...
class GradConv(tf.keras.Model):
    def __init__(self, J, m, n, Params, project = False, name = None):
        super(GradConv, self).__init__(name = name)
        self.J = J
        self.m = m
        self.n = n
        ...
        self.r = list()                    
        for j in range(J):
            self.RGrad.append(CNN(*Params[:6]))
    ...
    def call(self, A, k):
        ...
        for j in range(self.J):
            r = self.r[j](self.A@tf.linalg.diag(u), z, y)
            z = r+self.RGrad[j](z)   
            if self.project:
                z = tf.nn.relu(z)
        model = tf.keras.Model(inputs = [y, z_input, u], outputs = z, name = 'GradConv_{}'.format(k))
        return model